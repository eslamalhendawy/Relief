      ...
      <div className="flex items-center justify-between">
        ...
        <div className="relative">
          ...
          <ul className={`absolute bg-white border bottom-[-90px] left-0 w-[200px] text-center text-[#212529] font-semibold ${hidden && "hidden"}`}>
            <li onClick={() => setHidden(true)} className="py-2 cursor-pointer border-b-2 hover:bg-[#f6f3ef] duration-200">Care Giver Nearest To You</li>
            <li onClick={() => setHidden(true)} className="py-2 cursor-pointer hover:bg-[#f6f3ef] duration-200">Highest Rated</li>
          </ul>
        </div>
      </div>
      <div>
        {list.map((item, index) => (
          <div key={index} className="flex flex-col md:flex-row justify-between items-center gap-4 mt-8 bg-[#F6F3EF] hover:bg-white duration-200 cursor-pointer border border-[#F6F3EF] hover:border-[#00B4D8] p-6 rounded-xl">
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  {Array.from({ length: item.rating }).map((_, index) => (
                    <i key={index} className="fa-solid fa-star text-[#FFD700]" />
                  ))}
                </div>
              </div>
            </div>
